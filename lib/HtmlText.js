...
function isImageElement(tagName) : Boolean {
    return IMAGE_ELEMENTS.indexOf(tagName) != -1;
}
...
function processNode(node, parentKey, props) {
    ...
    if(isImageElement(nodeName)){
        ...
        return (
            <Image key={key} source={{uri:src}} style={{height:Dimensions.get('window').width*0.6,width:Dimensions.get('window').width,resizeMode:'resize'}}/>
    )
    }
    ...
    if (isInlineElement(nodeName)) {
        ...
        node.childNodes.forEach((child, index) => {
            if (isInlineElement(child.nodeName) || isText(child.nodeName) || isImageElement(child.nodeName)) {
